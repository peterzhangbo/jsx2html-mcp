...
function createMemoryHistory(options = {}) {
  ...
  let entries;
  entries = initialEntries.map((entry, index2) => createMemoryLocation(entry, typeof entry === "string" ? null : entry.state, index2 === 0 ? "default" : undefined, typeof entry === "string" ? undefined : entry.mask));
  ...
  function createMemoryLocation(to, state = null, key, mask) {
    let location2 = createLocation(entries ? getCurrentLocation().pathname : "/", to, state, key, mask);
...
var DataRoutes = class {
  ...
  commitHmrRoutes() {
    if (__privateGet(this, _hmrRoutes)) {
      ...
      __privateSet(this, _hmrRoutes, undefined);
      __privateSet(this, _hmrBranches, undefined);
    }
  }
};
_routes = new WeakMap;
...
function createRouter(init) {
  ...
  let manifest = {};
  let dataRoutes = new DataRoutes(convertRoutesToDataRoutes(init.routes, mapRouteProperties2, undefined, manifest));
  ...
  async function navigate(to, opts) {
    ...
    let nextLocation = createLocation(currentLocation, path, opts && opts.state, undefined, maskPath);
    nextLocation = {
      ...nextLocation,
  ...
  async function startNavigation(historyAction, location2, opts) {
    ...
    let matches = opts?.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? state.matches : matchRoutesImpl(routesToUse, location2, basename, false, dataRoutes.branches);
    let flushSync = (opts && opts.flushSync) === true;
    ...
    if (opts && opts.pendingError) {
      ...
      let actionResult = await handleAction(request, location2, opts.submission, matches, historyAction, scopedContext, fogOfWar.active, opts && opts.initialHydration === true, { replace: opts.replace, flushSync });
      ...
      loadingNavigation = getLoadingNavigation(location2, matches, historyAction, opts.submission);
      flushSync = false;
    ...
    let {
      ...
      errors,
      workingFetchers
    } = await handleLoaders(request, location2, matches, historyAction, scopedContext, fogOfWar.active, loadingNavigation, opts && opts.submission, opts && opts.fetcherSubmission, opts && opts.replace, opts && opts.initialHydration === true, flushSync, pendingActionResult, opts && opts.callSiteDefaultShouldRevalidate);
    if (shortCircuited) {
      return;
    }
    pendingNavigationController = null;
  ...
  async function handleAction(request, location2, submission, matches, historyAction, scopedContext, isFogOfWar, initialHydration, opts = {}) {
    interruptActiveLoads();
    let navigation = getSubmittingNavigation(location2, matches, historyAction, submission);
    ...
    if (!actionMatch.route.action && !actionMatch.route.lazy) {
      ...
      let dsMatches = getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location2, matches, actionMatch, initialHydration ? [] : hydrationRouteProperties2, scopedContext);
      let results = await callDataStrategy(request, location2, dsMatches, scopedContext, null);
    ...
    if (isRedirectResult(result)) {
      ...
      if (opts && opts.replace != null) {
        ...
        let location22 = normalizeRedirectLocation(result.response.headers.get("Location"), new URL(request.url), basename, init.history);
  ...
  async function handleLoaders(request, location2, matches, historyAction, scopedContext, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult, callSiteDefaultShouldRevalidate) {
    ...
    let { loaderResults, fetcherResults } = await callLoadersAndMaybeResolveData(dsMatches, revalidatingFetchers, request, location2, scopedContext);
    if (request.signal.aborted) {
      return { shortCircuited: true };
    }
    if (pendingNavigationController) {
  ...
  async function handleFetcherAction(key, routeId, path, requestMatches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission, callSiteDefaultShouldRevalidate) {
    ...
    let { loaderData, errors } = processLoaderData(state, matches, loaderResults, undefined, revalidatingFetchers, fetcherResults, finalFetchers);
  ...
  async function callLoadersAndMaybeResolveData(matches, fetchersToLoad, request, location2, scopedContext) {
    let loaderResultsPromise = callDataStrategy(request, location2, matches, scopedContext, null);
  ...
  function _internalSetRoutes(newRoutes) {
    manifest = {};
    dataRoutes.setHmrRoutes(convertRoutesToDataRoutes(newRoutes, mapRouteProperties2, undefined, manifest));
  }
...
function createStaticHandler(routes, opts) {
  ...
  let dataRoutes = convertRoutesToDataRoutes(routes, mapRouteProperties2, undefined, manifest);
  ...
  async function query(request, {
    ...
    let location2 = createLocation("", normalizePathImpl(request), null, "default");
    let matches = matchRoutesImpl(dataRoutes, location2, basename, false, routeBranches);
      ...
      try {
        ...
        let response = await runServerMiddlewarePipeline({
          ...
          let res = await generateMiddlewareResponse(async (revalidationRequest, opts2 = {}) => {
            let result2 = await queryImpl(revalidationRequest, location2, matches, requestContext, dataStrategy || null, skipLoaderErrorBubbling === true, null, "filterMatchesToLoad" in opts2 ? opts2.filterMatchesToLoad ?? null : filterMatchesToLoad ?? null, skipRevalidation === true);
  ...
  async function queryRoute(request, {
    ...
    if (generateMiddlewareResponse) {
      ...
      let response = await runServerMiddlewarePipeline({
        ...
        let res = await generateMiddlewareResponse(async (innerRequest) => {
          let result2 = await queryImpl(innerRequest, location2, matches, requestContext, dataStrategy || null, false, match, null, false);
    ...
    function handleQueryResult(result2) {
      ...
      let error = result2.errors ? Object.values(result2.errors)[0] : undefined;
      if (error !== undefined) {
        throw error;
      }
  ...
  async function queryImpl(request, location2, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad, skipRevalidation) {
    ...
    try {
      if (isMutationMethod(request.method)) {
        let result2 = await submit(request, location2, matches, routeMatch || getTargetMatch(matches, location2), requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch != null, filterMatchesToLoad, skipRevalidation);
        return result2;
      }
      let result = await loadRouteData(request, location2, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad);
  ...
  async function submit(request, location2, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest, filterMatchesToLoad, skipRevalidation) {
    ...
    if (!actionMatch.route.action && !actionMatch.route.lazy) {
      ...
      let dsMatches = getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location2, matches, actionMatch, [], requestContext);
      let results = await callDataStrategy(request, location2, dsMatches, isRouteRequest, requestContext, dataStrategy);
      result = results[actionMatch.route.id];
      if (request.signal.aborted) {
    ...
    if (isErrorResult(result)) {
      let boundaryMatch = skipLoaderErrorBubbling ? actionMatch : findNearestBoundary(matches, actionMatch.route.id);
      let handlerContext2 = await loadRouteData(loaderRequest, location2, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null, filterMatchesToLoad, [boundaryMatch.route.id, result]);
      return {
        ...handlerContext2,
        statusCode: isRouteErrorResponse(result.error) ? result.error.status : result.statusCode != null ? result.statusCode : 500,
        actionData: null,
    ...
    let handlerContext = await loadRouteData(loaderRequest, location2, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null, filterMatchesToLoad);
    return {
      ...handlerContext,
      actionData: {
        [actionMatch.route.id]: result.data
  ...
  async function loadRouteData(request, location2, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad, pendingActionResult) {
    ...
    let dsMatches;
    if (routeMatch) {
      dsMatches = getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location2, matches, routeMatch, [], requestContext);
    } else {
      ...
      dsMatches = matches.map((match, index) => {
        if (maxIdx != null && index > maxIdx) {
          return getDataStrategyMatch(mapRouteProperties2, manifest, request, location2, pattern, match, [], requestContext, false);
        }
        return getDataStrategyMatch(mapRouteProperties2, manifest, request, location2, pattern, match, [], requestContext, (match.route.loader || match.route.lazy) != null && (!filterMatchesToLoad || filterMatchesToLoad(match)));
      });
    }
  ...
  async function callDataStrategy(request, location2, matches, isRouteRequest, requestContext, dataStrategy) {
    let results = await callDataStrategyImpl(dataStrategy || defaultDataStrategy, request, location2, matches, null, requestContext, true);
    let dataResults = {};
...
function getMatchesToLoad(request, scopedContext, mapRouteProperties2, manifest, history, state, matches, submission, location2, lazyRoutePropertiesToSkip, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, hasPatchRoutesOnNavigation, branches, pendingActionResult, callSiteDefaultShouldRevalidate) {
  ...
  let dsMatches = matches.map((match, index) => {
    ...
    if (forceShouldLoad !== null) {
      return getDataStrategyMatch(mapRouteProperties2, manifest, request, location2, pattern, match, lazyRoutePropertiesToSkip, scopedContext, forceShouldLoad);
    }
    ...
    return getDataStrategyMatch(mapRouteProperties2, manifest, request, location2, pattern, match, lazyRoutePropertiesToSkip, scopedContext, shouldLoad, shouldRevalidateArgs, callSiteDefaultShouldRevalidate);
  });
...
function dataWithResponseInitToResponse(data2) {
  return Response.json(data2.data, data2.init ?? undefined);
}
...
function isPlainObject2(thing) {
  const proto = Object.getPrototypeOf(thing);
  return proto === Object.prototype || proto === null || Object.getOwnPropertyNames(proto).sort().join("\x00") === objectProtoNames2;
}
...
function getRouteComponents(route, routeModule, isSpaMode) {
  ...
  let ErrorBoundary = routeModule.ErrorBoundary ? routeModule.ErrorBoundary : route.id === "root" ? () => /* @__PURE__ */ React6.createElement(RemixRootDefaultErrorBoundary, { error: useRouteError() }) : undefined;
    ...
    if (routeModule) {
      ...
      dataRoute.lazy = {
        loader: route.hasClientLoader ? async () => {
          let { clientLoader } = route.clientLoaderModule ? await import(route.clientLoaderModule) : await getLazyRoute();
        ...
        action: route.hasClientAction ? async () => {
          let clientActionPromise = route.clientActionModule ? import(route.clientActionModule) : getLazyRoute();
          prefetchRouteModuleChunks(route);
        ...
        middleware: route.hasClientMiddleware ? async () => {
          let { clientMiddleware } = route.clientMiddlewareModule ? await import(route.clientMiddlewareModule) : await getLazyRoute();
...
function RemixRootDefaultErrorBoundary({
  ...
  return /* @__PURE__ */ React9.createElement(BoundaryShell, {
    ...
    isOutsideRemixApp
  }, /* @__PURE__ */ React9.createElement("h1", { style: { fontSize: "24px" } }, "Application Error"), /* @__PURE__ */ React9.createElement("pre", {
    style: {
      padding: "2rem",
...
var Link = React10.forwardRef(function LinkWithRef({
  ...
  let maskedHref = null;
  if (mask) {
    let resolved = resolveTo(mask, [], location2.mask ? location2.mask.pathname : "/", true);
    if (basename !== "/") {
...
function useSearchParams(defaultInit) {
  ...
  let location2 = useLocation();
  let searchParams = React10.useMemo(() => getSearchParamsForLocation(location2.search, hasSetSearchParamsRef.current ? null : defaultSearchParamsRef.current), [location2.search]);
  let navigate = useNavigate();
...
function useFetchers() {
  ...
  return React10.useMemo(() => Array.from(state.fetchers.entries()).map(([key, fetcher]) => ({
    ...fetcher,
    key
  })), [state.fetchers]);
}
...
function createRoutesStub(routes, _context) {
  return function RoutesTestStub({
    ...
    if (routerRef.current == null) {
      ...
      let patched = processRoutes(convertRoutesToDataRoutes(routes, (r) => r), _context !== undefined ? _context : future?.v8_middleware ? new RouterContextProvider : {}, frameworkContextRef.current.manifest, frameworkContextRef.current.routeModules);
...
function processRoutes(routes, context, manifest, routeModules, parentId) {
  return routes.map((route) => {
    ...
    let newRoute = {
      ...
      path: route.path,
      index: route.index,
      Component: route.Component ? withComponentProps(route.Component) : undefined,
      HydrateFallback: route.HydrateFallback ? withHydrateFallbackProps(route.HydrateFallback) : undefined,
      ErrorBoundary: route.ErrorBoundary ? withErrorBoundaryProps(route.ErrorBoundary) : undefined,
      action: route.action ? (args) => route.action({ ...args, context }) : undefined,
      loader: route.loader ? (args) => route.loader({ ...args, context }) : undefined,
      middleware: route.middleware ? route.middleware.map((mw) => (...args) => mw({ ...args[0], context }, args[1])) : undefined,
      handle: route.handle,
...
function createStaticHandlerDataRoutes(manifest, future, parentId = "", routesByParentId = groupRoutesByParentId2(manifest)) {
  return (routesByParentId[parentId] || []).map((route) => {
    let commonRoute = {
      ...
      action: route.module.action ? (args) => callRouteHandler(route.module.action, args, future) : undefined,
...
function prependCookies(parentHeaders, childHeaders) {
  ...
  if (parentSetCookieString) {
    let cookies = import_set_cookie_parser.splitCookiesString(parentSetCookieString);
...
async function handleSingleFetchRequest(serverMode, build, staticHandler, request, normalizedPath, loadContext, handleError) {
  ...
  let response = isMutationMethod(request.method) ? await singleFetchAction(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) : await singleFetchLoaders(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError);
  return response;
}
...
async function routeRSCServerRequest({
  ...
  const getPayload = () => {
    ...
    return Object.defineProperties(payloadPromise, {
      ...
      formState: {
        get() {
          return payloadPromise.then((payload) => payload.type === "render" ? payload.formState : undefined);
        }
      }
    });
  };
...
function createHydratedRouter({
  ...
  if (window.history.state && window.history.state.masked) {
    window.history.replaceState({ ...window.history.state, masked: undefined }, "");
  }
...
function HydratedRouter(props) {
  ...
  let [criticalCss, setCriticalCss] = React23.useState(ssrInfo?.context.criticalCss);
  React23.useEffect(() => {
    if (true) {
  ...
  return /* @__PURE__ */ React23.createElement(React23.Fragment, null, /* @__PURE__ */ React23.createElement(FrameworkContext.Provider, {
    value: {
  ...
  }, /* @__PURE__ */ React23.createElement(RemixErrorBoundary, { location: location2 }, /* @__PURE__ */ React23.createElement(RouterProvider2, {
    router,
