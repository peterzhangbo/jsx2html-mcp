...
var require_fast_equals = __commonJS((exports, module) => {
  (function(global, factory) {
    ...
    function createComparator(_a) {
      var { areArraysEqual: areArraysEqual2, areDatesEqual: areDatesEqual2, areMapsEqual: areMapsEqual2, areObjectsEqual: areObjectsEqual2, areRegExpsEqual: areRegExpsEqual2, areSetsEqual: areSetsEqual2, createIsNestedEqual } = _a;
...
var require_shims = __commonJS((exports) => {
...
var require_getPrefix = __commonJS((exports) => {
  Object.defineProperty(exports, "__esModule", {
    value: true
  });
...
var require_domFns = __commonJS((exports) => {
  Object.defineProperty(exports, "__esModule", {
    value: true
  });
...
var require_positionFns = __commonJS((exports) => {
  Object.defineProperty(exports, "__esModule", {
    value: true
  });
...
var require_DraggableCore = __commonJS((exports) => {
  Object.defineProperty(exports, "__esModule", {
    value: true
  });
  exports.default = undefined;
...
import * as react_dom2 from "react-dom";
var require_Draggable = __commonJS((exports) => {
  ...
  var _propTypes = _interopRequireDefault(require_prop_types());
  var _reactDom = _interopRequireDefault(react_dom2);
...
var require_utils = __commonJS((exports) => {
  ...
  var _react = _interopRequireDefault(react3);
  function _interopRequireDefault(e) {
    return e && e.__esModule ? e : { default: e };
  }
...
var require_propTypes = __commonJS((exports) => {
  exports.__esModule = true;
  exports.resizableProps = undefined;
  var _propTypes = _interopRequireDefault(require_prop_types());
...
var require_Resizable = __commonJS((exports) => {
  exports.__esModule = true;
  exports.default = undefined;
  var React = _interopRequireWildcard(react4);
  var _reactDraggable = require_cjs();
  ...
  class Resizable extends React.Component {
    ...
    resizeHandler(handlerName, axis) {
      return (e, _ref2) => {
        ...
        if (axisV === "n")
          deltaY = -deltaY;
        const baseWidth = (_this$lastSize$width = (_this$lastSize = this.lastSize) == null ? undefined : _this$lastSize.width) != null ? _this$lastSize$width : this.props.width;
        const baseHeight = (_this$lastSize$height = (_this$lastSize2 = this.lastSize) == null ? undefined : _this$lastSize2.height) != null ? _this$lastSize$height : this.props.height;
...
var require_ResizableBox = __commonJS((exports) => {
  exports.__esModule = true;
  exports.default = undefined;
  var React = _interopRequireWildcard(react5);
  var _propTypes = _interopRequireDefault(require_prop_types());
...
var require_react_resizable = __commonJS((exports, module) => {
...
function moveElementAwayFromCollision(layout, collidesWith, itemToMove, isUserAction, compactType, cols) {
  ...
  if (isUserAction) {
    ...
    const collisionNorth = firstCollision !== undefined && firstCollision.y + firstCollision.h > collidesWith.y;
    const collisionWest = firstCollision !== undefined && collidesWith.x + collidesWith.w > firstCollision.x;
    if (!firstCollision) {
    ...
    if (collisionNorth && compactV) {
      return moveElement(layout, itemToMove, undefined, itemToMove.y + 1, isUserAction, preventCollision, compactType);
    }
...
function useGridLayout(options) {
  ...
  useEffect(() => {
    if (isDraggingRef.current)
      return;
    if (!import_fast_equals.deepEqual(propsLayout, prevLayoutRef.current)) {
      setLayout(propsLayout);
    }
  }, [propsLayout, setLayout]);
  useEffect(() => {
    if (!import_fast_equals.deepEqual(layout, prevLayoutRef.current)) {
      prevLayoutRef.current = layout;
      onLayoutChange?.(layout);
    }
  }, [layout, onLayoutChange]);
  const onDragStart = useCallback((itemId, x, y) => {
  ...
  const onDrag = useCallback((itemId, x, y) => {
    const item = getLayoutItem(layout, itemId);
    if (!item)
      return;
    setDragState((prev) => ({
  ...
  const onDragStop = useCallback((itemId, x, y) => {
    const item = getLayoutItem(layout, itemId);
    if (!item)
      return;
    const newLayout = moveElement(layout, item, x, y, true, preventCollision, compactor.type, cols, compactor.allowOverlap);
  ...
  const onResizeStart = useCallback((itemId) => {
    const item = getLayoutItem(layout, itemId);
    if (!item)
      return null;
    setResizeState({
  ...
  const onResize = useCallback((itemId, w, h, x, y) => {
  ...
  const onResizeStop = useCallback((itemId, w, h) => {
    onResize(itemId, w, h);
    setResizeState({
      resizing: false,
      oldResizeItem: null,
  ...
  const onDropDragOver = useCallback((droppingItem, position) => {
  ...
  const onDrop = useCallback((droppingItem) => {
    const newLayout = layout.map((item) => {
...
function useResponsiveLayout(options) {
  ...
  const initialBreakpoint = useMemo(() => getBreakpointFromWidth(breakpoints, width), []);
  const initialCols = useMemo(() => getColsFromBreakpoint(initialBreakpoint, colsConfig), [initialBreakpoint, colsConfig]);
  ...
  useEffect(() => {
    if (!import_fast_equals.deepEqual(propsLayouts, prevPropsLayoutsRef.current)) {
      setLayouts(propsLayouts);
      prevPropsLayoutsRef.current = propsLayouts;
    }
  }, [propsLayouts, setLayouts]);
  useEffect(() => {
    if (!import_fast_equals.deepEqual(layouts, prevLayoutsRef.current)) {
...
import React2, { useState as useState2, useRef as useRef2, useMemo as useMemo2, useCallback as useCallback2, useEffect as useEffect2 } from "react";
...
// node_modules/react-grid-layout/dist/chunk-WGL5FSZH.mjs
var import_fast_equals2 = __toESM(require_fast_equals(), 1);
...
function GridItem(props) {
  ...
  const dragPositionRef = useRef2({ left: 0, top: 0 });
  const resizePositionRef = useRef2({
    top: 0,
    left: 0,
    width: 0,
    height: 0
  });
  const prevDroppingPositionRef = useRef2(undefined);
  const layoutRef = useRef2(layout);
  layoutRef.current = layout;
  const onDragStartRef = useRef2(null);
  const onDragRef = useRef2(null);
  const dragPendingRef = useRef2(false);
  const initialDragClientRef = useRef2({ x: 0, y: 0 });
  const thresholdExceededRef = useRef2(false);
  const positionParams = useMemo2(() => ({
    cols,
    containerPadding,
    containerWidth,
    margin,
  ...
  const constraintContext = useMemo2(() => ({
    cols,
    maxRows,
    containerWidth,
    containerHeight: 0,
  ...
  const getConstraintContext = useCallback2(() => ({
    ...constraintContext,
    layout: layoutRef.current
  }), [constraintContext]);
  const effectiveLayoutItem = useMemo2(() => layoutItem ?? {
    i,
    x,
    y,
    w,
  ...
  const createStyle = useCallback2((pos2) => {
  ...
  const onDragStart = useCallback2((e, { node }) => {
    if (!onDragStartProp)
      return;
  ...
  const onDrag = useCallback2((e, { node, deltaX, deltaY }) => {
  ...
  const onDragStop = useCallback2((e, { node }) => {
  ...
  const onResizeHandler = useCallback2((e, { node, size, handle: resizeHandle2 }, position, handlerName) => {
    const handler = handlerName === "onResizeStart" ? onResizeStartProp : handlerName === "onResize" ? onResizeProp : onResizeStopProp;
    if (!handler)
      return;
    let updatedSize;
    if (node) {
      updatedSize = resizeItemInDirection(resizeHandle2, position, size, containerWidth);
    } else {
      updatedSize = {
        ...size,
        top: position.top,
  ...
  const handleResizeStart = useCallback2((e, data) => {
    setResizing(true);
  ...
  const handleResize = useCallback2((e, data) => {
    const pos2 = calcGridItemPosition(positionParams, x, y, w, h);
    const typedData = {
      ...data,
      handle: data.handle
  ...
  const handleResizeStop = useCallback2((e, data) => {
    setResizing(false);
  ...
  newChild = /* @__PURE__ */ jsx(import_react_resizable.Resizable, {
  ...
  newChild = /* @__PURE__ */ jsx(import_react_draggable.DraggableCore, {
...
function synchronizeLayoutWithChildren(initialLayout, children, cols, compactor) {
  ...
  React2.Children.forEach(children, (child) => {
    if (!React2.isValidElement(child) || child.key === null)
      return;
...
function GridLayout(props) {
  ...
  const gridConfig = useMemo2(() => ({ ...defaultGridConfig, ...gridConfigProp }), [gridConfigProp]);
  const dragConfig = useMemo2(() => ({ ...defaultDragConfig, ...dragConfigProp }), [dragConfigProp]);
  const resizeConfig = useMemo2(() => ({ ...defaultResizeConfig, ...resizeConfigProp }), [resizeConfigProp]);
  const dropConfig = useMemo2(() => ({ ...defaultDropConfig, ...dropConfigProp }), [dropConfigProp]);
  ...
  const droppingItem = useMemo2(() => droppingItemProp ?? {
    i: "__dropping-elem__",
    ...defaultDropItem
  ...
  const [layout, setLayout] = useState2(() => synchronizeLayoutWithChildren(propsLayout, children, cols, compactor));
  const [activeDrag, setActiveDrag] = useState2(null);
  const [resizing, setResizing] = useState2(false);
  const [droppingDOMNode, setDroppingDOMNode] = useState2(null);
  const [droppingPosition, setDroppingPosition] = useState2();
  const oldDragItemRef = useRef2(null);
  const oldResizeItemRef = useRef2(null);
  const oldLayoutRef = useRef2(null);
  const dragEnterCounterRef = useRef2(0);
  const prevLayoutRef = useRef2(layout);
  const prevPropsLayoutRef = useRef2(propsLayout);
  const prevChildrenRef = useRef2(children);
  const prevCompactTypeRef = useRef2(compactType);
  const layoutRef = useRef2(layout);
  layoutRef.current = layout;
  useEffect2(() => {
    setMounted(true);
    if (!import_fast_equals2.deepEqual(layout, propsLayout)) {
      onLayoutChange(layout);
    }
  }, []);
  useEffect2(() => {
    ...
    if (droppingDOMNode)
      return;
    const layoutChanged = !import_fast_equals2.deepEqual(propsLayout, prevPropsLayoutRef.current);
    ...
    if (layoutChanged || childrenChanged || compactTypeChanged) {
      ...
      if (!import_fast_equals2.deepEqual(newLayout, layout)) {
        setLayout(newLayout);
      }
    }
  ...
  useEffect2(() => {
    if (!activeDrag && !import_fast_equals2.deepEqual(layout, prevLayoutRef.current)) {
      prevLayoutRef.current = layout;
  ...
  const containerHeight = useMemo2(() => {
    if (!autoSize)
      return;
  ...
  const onDragStart = useCallback2((i, _x, _y, data) => {
  ...
  const onDrag = useCallback2((i, x, y, data) => {
    const currentLayout = layoutRef.current;
  ...
  const onDragStop = useCallback2((i, x, y, data) => {
    if (!activeDrag)
      return;
    const currentLayout = layoutRef.current;
    const oldDragItem = oldDragItemRef.current;
    ...
    if (oldLayout && !import_fast_equals2.deepEqual(oldLayout, finalLayout)) {
  ...
  const onResizeStart = useCallback2((i, _w, _h, data) => {
    const currentLayout = layoutRef.current;
    const l = getLayoutItem(currentLayout, i);
    if (!l)
      return;
  ...
  const onResize = useCallback2((i, w, h, data) => {
    const currentLayout = layoutRef.current;
    ...
    if (shouldMoveItem && newX !== undefined && newY !== undefined) {
      finalLayout = moveElement(newLayout, l, newX, newY, true, preventCollision, compactType, cols, allowOverlap);
    }
    const placeholder = {
      w: l.w,
      h: l.h,
  ...
  const onResizeStop = useCallback2((i, _w, _h, data) => {
    const currentLayout = layoutRef.current;
    const oldResizeItem = oldResizeItemRef.current;
    const l = getLayoutItem(currentLayout, i);
  ...
  const removeDroppingPlaceholder = useCallback2(() => {
    const currentLayout = layoutRef.current;
  ...
  const processGridItem = useCallback2((child, isDroppingItem) => {
    if (!child || !child.key)
      return null;
...
function ResponsiveGridLayout(props) {
  ...
  const [breakpoint, setBreakpoint] = useState2(initialBreakpoint);
  const [cols, setCols] = useState2(initialCols);
  const [layout, setLayout] = useState2(initialLayout);
  const [layouts, setLayouts] = useState2(propsLayouts);
  const prevWidthRef = useRef2(width);
  const prevBreakpointRef = useRef2(propBreakpoint);
  const prevBreakpointsRef = useRef2(breakpoints);
  const prevColsRef = useRef2(colsConfig);
  const prevLayoutsRef = useRef2(propsLayouts);
  const prevCompactTypeRef = useRef2(compactType);
  const layoutsRef = useRef2(layouts);
  useEffect2(() => {
    layoutsRef.current = layouts;
  }, [layouts]);
  const derivedLayout = useMemo2(() => {
    if (!import_fast_equals2.deepEqual(propsLayouts, prevLayoutsRef.current)) {
      return findOrGenerateResponsiveLayout(propsLayouts, breakpoints, breakpoint, breakpoint, cols, compactor);
    }
    return null;
  ...
  useEffect2(() => {
    ...
    const breakpointsChanged = !import_fast_equals2.deepEqual(breakpoints, prevBreakpointsRef.current);
    const colsChanged = !import_fast_equals2.deepEqual(colsConfig, prevColsRef.current);
  ...
  const handleLayoutChange = useCallback2((newLayout) => {
