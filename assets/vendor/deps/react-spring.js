...
function usePrev(value) {
  const prevRef = useRef4(undefined);
  useEffect4(() => {
...
import { useContext as useContext2, useMemo as useMemo2, useRef as useRef7 } from "react";
...
var withAnimated = (Component, host) => {
  ...
  return forwardRef((givenProps, givenRef) => {
    ...
    const observerRef = useRef5(undefined);
    useIsomorphicLayoutEffect(() => {
...
import { useContext as useContext3, useRef as useRef22, useMemo as useMemo3 } from "react";
...
function useSprings(length, props, deps) {
  ...
  if (propsFn && !deps)
    deps = [];
  const ref = useMemo2(() => propsFn || arguments.length == 3 ? SpringRef() : undefined, []);
  const layoutId = useRef7(0);
  const forceUpdate = useForceUpdate();
  ...
  const ctrls = useRef7([...state.ctrls]);
  const updates = useRef7([]);
...
var useSpringRef = () => useState4(initSpringRef)[0];
...
function useTransition(data, props, deps) {
  ...
  const ref = useMemo3(() => propsFn || arguments.length == 3 ? SpringRef() : undefined, []);
  ...
  const renderTransitions = (render) => /* @__PURE__ */ React22.createElement(React22.Fragment, null, transitions.map((t, i) => {
    ...
    return elem && elem.type ? /* @__PURE__ */ React22.createElement(elem.type, { ...props2, key, ref: elemRef }) : elem;
  }));
...
function useInView(props, args) {
  const [isInView, setIsInView] = useState22(false);
  const ref = useRef32(undefined);
  const propsFn = is.fun(props) && props;
