...
var supportsViewTimeline = /* @__PURE__ */ memoSupports(() => window.ViewTimeline !== undefined, "viewTimeline");
...
var msToFps = (ms) => Math.round(1000 / ms);
...
class DOMVisualElement extends VisualElement {
  ...
  getBaseTargetFromProps(props, key) {
    ...
    return style2 ? style2[key] : undefined;
  }
...
function scrapeMotionValuesFromProps(props, prevProps, visualElement) {
  ...
  for (const key in style2) {
    if (isMotionValue(style2[key]) || prevStyle && isMotionValue(prevStyle[key]) || isForcedMotionValue(key, props) || visualElement?.getValue(key)?.liveStyle !== undefined) {
...
function usePresence(subscribe = true) {
  ...
  const safeToRemove = useCallback2(() => subscribe && onExitComplete && onExitComplete(id2), [id2, onExitComplete, subscribe]);
...
var AnimatePresence = ({ children, custom, initial = true, onExitComplete, presenceAffectsLayout = true, mode = "sync", propagate = false, anchorX = "left", anchorY = "top", root }) => {
  ...
  const exitingComponents = useRef3(new Set);
  ...
  return jsx3(Fragment, { children: renderedChildren.map((child) => {
    ...
    return jsx3(PresenceChild, { isPresent: isPresent2, initial: !isInitialRender.current || initial ? undefined : false, custom, presenceAffectsLayout, mode, root, onExitComplete: isPresent2 ? undefined : onExit, anchorX, anchorY, children: child }, key);
  }) });
};
...
function createMotionComponent(Component2, { forwardMotionProps = false, type } = {}, preloadedFeatures, createVisualElement) {
  ...
  function MotionDOMComponent(props, externalRef) {
    ...
    return jsxs(MotionContext.Provider, { value: context, children: [MeasureLayout && context.visualElement ? jsx7(MeasureLayout, { visualElement: context.visualElement, ...configAndProps }) : null, useRender(Component2, props, useMotionRef(visualState, context.visualElement, externalRef), visualState, isStatic, forwardMotionProps, isSVG)] });
  }
...
var createDomVisualElement = (Component2, options) => {
  ...
  return isSVG ? new SVGVisualElement(options) : new HTMLVisualElement(options, {
    allowProjection: Component2 !== Fragment3
  });
};
...
class MeasureLayoutWithContext extends Component2 {
  ...
  getSnapshotBeforeUpdate(prevProps) {
    ...
    if (drag || prevProps.layoutDependency !== layoutDependency || layoutDependency === undefined || prevProps.isPresent !== isPresent2) {
      projection.willUpdate();
    } else {
      this.safeToRemove();
    }
...
function useCycle(...items) {
  ...
  const runCycle = useCallback7((next2) => {
